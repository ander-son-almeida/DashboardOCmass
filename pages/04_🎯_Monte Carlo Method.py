...
###############################################################################
        ...
        with results:
            ...
            with col11:
                st.write("Integrated:")
                st.write(r"$M_{{wd}} = {}~M_{{\odot}}$ (White dwarf)".format((inv_mass_wd)))
                st.write(r"$M_{{total}} = {} \pm {}~M_{{\odot}}$".format(total_mass_integrated, int(total_mass_integrated*0.20)))
                
                st.write('---')
                
                
                st.write("Deitaled:")
                st.write(r"$M_{{sing}} = {}~M_{{\odot}}$ (Single stars)".format((mass_sing + inv_mass_sing)))
                st.write(r"$M_{{pri}} = {}~M_{{\odot}}$ (Primary stars)".format((mass_prim + inv_mass_prim)))
                st.write(r"$M_{{sec}} = {}~M_{{\odot}}$ (Secondary stars) ".format((mass_sec + inv_mass_sec)))
                
                st.write(r"$M_{{wd-s}} = {}~M_{{\odot}}$ (White dwarf - single)".format((inv_mass_wd_sing)))
                st.write(r"$M_{{wd-p}} = {}~M_{{\odot}}$ (White dwarf - primary)".format((inv_mass_wd_prim)))
                st.write(r"$M_{{wd-s}} = {}~M_{{\odot}}$ (White dwarf - secondary)".format((inv_mass_wd_sec)))
                st.write(r"$M_{{total}} = {} \pm {}~M_{{\odot}}$".format(total_mass_detailed, int(total_mass_detailed*0.20)))
