...
###############################################################################
        ...
        with results:
            ...
            with col11:
                st.write("Integrated:")
                st.write("$M_{{wd}} = {}~M_{{\odot}}$ (White dwarf)".format(int(inv_mass_wd)))
                ...
                st.write("Deitaled:")
                st.write("$M_{{sing}} = {}~M_{{\odot}}$ (Single stars)".format(int(mass_sing + inv_mass_sing)))
                st.write("$M_{{pri}} = {}~M_{{\odot}}$ (Primary stars)".format(int(mass_prim + inv_mass_prim)))
                st.write("$M_{{sec}} = {}~M_{{\odot}}$ (Secondary stars) ".format(int(mass_sec + inv_mass_sec)))
                
                st.write("$M_{{wd-s}} = {}~M_{{\odot}}$ (White dwarf - single)".format(int(inv_mass_wd_sing)))
                st.write("$M_{{wd-p}} = {}~M_{{\odot}}$ (White dwarf - primary)".format(int(inv_mass_wd_prim)))
                st.write("$M_{{wd-s}} = {}~M_{{\odot}}$ (White dwarf - secondary)".format(int(inv_mass_wd_sec)))
