...
###############################################################################
        ...
        with results:
            ...
            with col11:
                ...
                st.write("Deitaled:")
                st.write("$M_{{sing}} = {}~M_{{\odot}}$ (Single stars)".format((mass_sing + inv_mass_sing)))
                st.write("$M_{{pri}} = {}~M_{{\odot}}$ (Primary stars)".format((mass_prim + inv_mass_prim)))
                st.write("$M_{{sec}} = {}~M_{{\odot}}$ (Secondary stars) ".format((mass_sec + inv_mass_sec)))
                
                st.write("$M_{{wd-s}} = {}~M_{{\odot}}$ (White dwarf - single)".format((inv_mass_wd_sing)))
                st.write("$M_{{wd-p}} = {}~M_{{\odot}}$ (White dwarf - primary)".format((inv_mass_wd_prim)))
                st.write("$M_{{wd-s}} = {}~M_{{\odot}}$ (White dwarf - secondary)".format((inv_mass_wd_sec)))
